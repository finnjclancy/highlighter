# Highlight share, AI, and agent Worker
...
`/mcp` exposes 24 stateless Streamable HTTP tools spanning active-page state, page-ranged text extraction from PDFs open in Highlight's reader, exact and selection-based highlighting, staged Library selections, folder listing and reversible organisation, cross-library search, page listing, reversible update/remove/restore operations, page notes, context snapshots, Markdown/JSON/CSV/BibTeX/RIS export, summary and comparison source bundles, managed live links, gallery collaboration, context retrieval, and opening a saved passage. The public endpoint uses OAuth 2.1 discovery, dynamic client registration, authorization code + PKCE, hourly access tokens, and rotating 30-day refresh tokens. A 10-minute one-time code generated by the extension pairs ChatGPT to the correct browser without exposing the browser's private token.

`/.well-known/oauth-protected-resource`, `/.well-known/oauth-authorization-server`, `/oauth/register`, `/oauth/authorize`, `/oauth/token`, and `/oauth/revoke` make up the public authorization surface. Pairing codes, authorization codes, and refresh tokens use per-credential `OAuthSession` Durable Objects for strongly consistent one-time consumption. OAuth access tokens are HMAC-signed with the encrypted `OAUTH_SIGNING_SECRET` Worker secret and contain only a hashed bridge identifier. Registered public-client metadata remains in KV.

The extension's locally generated token authenticates its WebSocket at `/api/agent/socket`. A per-token `AgentBridge` Durable Object relays each tool call to the paired Chrome extension and returns its immediate result; PDF text, page text, library data, and created highlights are not stored by the bridge. `/mcp?token=<private-token>` remains supported for local development and must never be published.
...
Before the first OAuth deployment, create the signing secret with `npx wrangler secret put OAUTH_SIGNING_SECRET`. Deploy the Worker, add `https://highlighter-share.finnjclancy.workers.dev/mcp` to an OAuth-capable MCP client, then use **? → Pair ChatGPT** in the Highlight popup. Disconnecting closes the socket and removes the local token.
