...
`/mcp?token=<private-token>` exposes 20 stateless Streamable HTTP tools spanning active-page state, exact and selection-based highlighting, cross-library search, page listing, reversible update/remove/restore operations, bulk tags, page notes, context snapshots, Markdown/JSON/CSV/BibTeX/RIS export, summary and comparison source bundles, managed live links, gallery collaboration, context retrieval, and opening a saved passage. The same locally generated token authenticates the extension's WebSocket at `/api/agent/socket`. A per-token `AgentBridge` Durable Object relays each tool call to the paired Chrome extension and returns its immediate result; page text, library data, and created highlights are not stored by the bridge.
